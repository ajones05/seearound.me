					...
					if (msg && msg.errors){
						...
						if (msg.replyData){
							...
							for (x in msg.replyData){
								...
								html +='><div class="desc-frist"><img src="' + msg.replyData[x].sender_image +
									'"></div><div class="desc-second"><b style="color: #4276cd;font-size:16px;">'+(msg.replyData[x]).name+
									'</b><span>'+(msg.replyData[x]).reply_text+'</div><div class="desc-third"><span class="dateTime">'+
									(msg.replyData[x]).created+'</span></div><div class="clr"></div></li>';
							}
						...
						html += '<li class="replyTxt">'+ '<span  style="margin-left:2px;" class="errors" id="error_'+(msg.inboxData).id+'"></span>'+
							...
							'</span> <span><img id="rpl_loading_'+(msg.inboxData).id+'" style="display:none;margin-bottom: -6px;" src="'+baseUrl+
								'www/images/wait.gif" /></span>'+
							'</li>' +
						...
						html +='" />'+
							'<input type="hidden" id="reply_subject_'+(msg.inboxData).id+'" name="reply_subject_'+(msg.inboxData).id+
								'" value="'+(msg.inboxData).subject+'" />'+ '<div>'+'<textarea id="reply_text_area_'+(msg.inboxData).id+
								'" placeholder="Enter your message"></textarea></div>'+
							'</li>'+
							'<li class="btn"><input id="forFocus_'+number+'" type="submit" value="Reply" onclick="sendReply(this, '+
							(msg.inboxData).id+
			...
			success: function(msg){
				...
				if (msg && msg.errors){
					...
					if (stripmessage.length <= 185){
						var html = '<li class="desc" style="border-top: 0px solid #E9EAEB;"><b style="color: #0069D6;font-size:12px;">'+
							(msg.replyData[0]).name+'</b><span class="dateTime">'+(msg.replyData[0]).created+
							'</span><div style="padding:4px 4px 4px 0;">'+(msg.replyData[0]).reply_text+'</div><div class="clr"></div></li>';
					} else {
						...
						var html = '<li class="desc" style="border-top: 0px solid #E9EAEB;"><b style="color: #0069D6;font-size:12px;">'+
							(msg.replyData[0]).name+'</b><span class="dateTime">'+(msg.replyData[0]).created+
							'</span><div style="padding:4px 4px 4px 0;">'+stringArray[0]+'<br/>'+stringArray[1]+
							'<br/></div><div class="clr"></div></li>';
					}
...
function showAllReply(thisone, id){
	...
	$.ajax({
		...
		success: function (msg){
			...
			for (x in msg.replyData){
				html += '<li class="desc" style="border-top: 0px none;;"><b style="color: #0069D6;font-size:12px;">'+
					(msg.replyData[x]).name+'</b><span class="dateTime">'+(msg.replyData[x]).created+
					'</span><div style="padding:4px 4px 4px 0;">'+(msg.replyData[x]).reply_text+'</div><div class="clr"></div></li>';
			}
