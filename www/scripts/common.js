	 ...
	 $.ajaxSetup({async:false});
        ...
        $.post(
            ...
            function(obj){
                ...
                var pagingDiv = '<div id="pagingDiv">'+obj.paging+'</div>';
                ...
                var NoOfResult = 0;
               	...
               	for(i in resultObj) {
               	    ...
               	    if(NoOfResult<15){
                        ...
                        var data = {name:resultObj[i]['Name'],id:resultObj[i]['id'],news:resultObj[i]['news'],userImage:resultObj[i]['Profile_image'],user_id:resultObj[i]['user_id']};

                        if(commonMap.createMarker(point,UserMarker1,'other',data)){
        					...
        					if (results[0]) {
                                ...
                                if($('#fileNm').html() != ""){  
                                    ...
                                    $('#loading').hide();
	           			   			...
	           			   			$.post(baseUrl+'home/add-news', {'news': news,'latitude': userLatitude,'longitude': userLongitude, 'address': formattedAddress}, showAddNews, "html");
...
function showAddNews(obj){
    ...
    var jsdata = $.parseJSON(obj);
        ...
        var html = '<div class="scrpBox" onmouseover="toggleBounce('+jsdata.id+','+user_id+');" onmouseout="stopBounce('+jsdata.id+','+user_id+');"><ul class="myScrp afterClr" id="newsDiv'+jsdata.id+'">'+ 
    		...
    		if(jsdata.image.indexOf('://') > 0) {
    			...
    			html += '<img src="'+baseUrl+'uploads/' + jsdata.image + '"  style="width:35px;height:35px;"/></a></span>';
    						 ...
    						 html+='</li>'+
                                ...
                                html+=news.substring(0,350);

                                if(news.length>350) {

                                     html+='<span><a id="moreButton_'+jsdata.id+'" href="javascript:void(0)" onclick="showMoreNews(this,'+jsdata.id+')">....More</a><span id="morecontent_'+jsdata.id+'" style="display:none">'+news.substring(350,news.length)+'</span></span>';
...
//            hideShowPreviousNextButton(['prevDiv_','nextDiv_'],['none','none'],bubbleArrayIndex); //'block' in previous version
 ...
 //           hideShowPreviousNextButton(['prevDiv_','nextDiv_'],['none','none'],bubbleArrayIndex); //'block' in previous version
    ...
    if(currentBubble.contentArgs[0][2] == 'No News'){
    ...
    if(res){
        ...
        for(jsdataCounter in res){
            ...
            if((jsdataCounter>=startPage)&&(jsdataCounter<=endPage)) {
                ...
                var html = '<div class="scrpBox" onmouseover="toggleBounce('+jsdata.id+','+jsdata.user_id+');" onmouseout="stopBounce('+jsdata.id+','+user_id+');"><ul class="myScrp afterClr" id="newsDiv'+jsdata.id+'" >'+ 
            		...
            		if(jsdata.Profile_image.indexOf('://') > 0) {
            			...
            			html += '<img src="'+baseUrl+'uploads/' + jsdata.Profile_image + '"  style="width:35px;height:35px;"/></a>';
                                 ...
                                 html+='</li>'+
										...
										html+=jsdata.news.substring(0,255);

                                        if(jsdata.news.length>255) {

                                             html+='<span><a id="moreButton_'+jsdata.id+'" href="javascript:void(0)" onclick="showMoreNews(this,'+jsdata.id+')">....More</a><span id="morecontent_'+jsdata.id+'" style="display:none">'+jsdata.news.substring(255,jsdata.news.length)+'</span></li>';
         ...
         if(!paginationFlag){
                ...
                paginationDiv='<div id="pagingDiv"><div class="postClass" align="center"  onclick="paging('+pageNumber+');"> <b>Older Posts</b> </div><div style="display:none;" class="postClass" align="center"><img src="'+baseUrl+'www/images/wait.gif" /></div></div>';
               ...
               //$.ajaxSetup({async:true});
				$.post(
					...
					function(obj){
						...
						var commentData = '<li id="comment_'+obj.commentId +'" class="cmntList afterClr"><ul><li class="imgPro">';
						...
						commentData += '</li><li class="title">' + userName + '</li><li class="cmnt">'+comments.substring(0,255);
...
function searchData(user_id,flag,filter,circleCenterFlag) {
    ...
    var url = baseUrl+"home/search-news/";

    var circleLat = map.getCenter().lat();

    var circleLong = map.getCenter().lng();
...
function changeCircle() { //alert(456);
...
function sliderInitialization(){
    ...
    $("#slider").slider({max:1.5,min:.5,step:0.1,value:circleSize,animate: true}).bind( "slidestop", function(event, ui) { if(ui.value == 1)
            ...
            ui.value = "1.0";circleSize = Number(ui.value);changeCircle();});
